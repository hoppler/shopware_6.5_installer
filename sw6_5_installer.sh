...
### This script has been written by Torsten Schürmann which is the copyright holder.
...
install_part_1() {
  ...
  while true; do
    ...
    if [ "$pass1" == "$pass2" ]; then
      ...
      if [ "$language" = "en" ]; then
        ...
        show_error "Passwörter stimmen nicht überein. Bitte versuchen Sie es erneut."
      fi
    fi
  done
  ...
  if [ "$(id -u)" -ne 0 ]; then
    if [ "$language" = "en" ]; then
      ...
      show_error "Dieses Skript erfordert Root-Rechte. Bitte führen Sie es mit sudo aus."
    fi
    return 1
  fi
...
EOF
  ...
  if [ "$language" = "en" ]; then
    ...
    show_info "Bitte führen Sie den folgenden Befehl in Ihrem Browser aus: $ipaddress/$filename\n\nWarten Sie auf 'File not found.' und führen Sie dann Teil 2 aus."
  fi
}
...
install_part_2() {
  ...
  if [ "$language" = "en" ]; then
    ...
    show_info "Teil 2 ist abgeschlossen.\n\nBitte rufen Sie $ipaddress in Ihrem Browser auf, um die Installation abzuschließen."
  fi
}
...
uninstall_shopware() {
  if [ "$language" = "en" ]; then
    ...
    if (whiptail --title "Bestätigung" --yesno "Möchten Sie Shopware wirklich deinstallieren?" 8 50) then
      rm -rf /var/www/html/*
...
while true; do
  ...
  if [ -f "/var/www/html/public/composer.lock" ]; then
    if [ "$language" = "en" ]; then
      ...
      choice=$(whiptail --title "Shopware Installation" --menu "Wählen Sie eine Option aus:" 12 50 3 \
  ...
  if [ -f "/var/www/html/composer.lock" ]; then
    if [ "$language" = "en" ]; then
      ...
      choice=$(whiptail --title "Shopware Installation" --menu "Wählen Sie eine Option aus:" 12 50 4 \
        "u" "Shopware deinstallieren" \
        "e" "Beenden" 3>&1 1>&2 2>&3)
    fi
  else
